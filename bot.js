...
// Fungsi untuk menangani kode status 429 Too Many Requests
async function handleTooManyRequests(delayInSeconds){ 
    const delayInMilliseconds=delayInSeconds *1000; 
    coloredLog(`Terlalu banyak permintaan! Menunggu selama${delayInSeconds} detik sebelum mencoba lagi...`,"WARN"); 
    await new Promise(resolve => setTimeout(resolve ,delayInMilliseconds));
}

async function getUserStats(tokens ,config,retryCount=0){ 
    try { 
        coloredLog('Mengambil statistik pengguna...' ,"INFO");
        ...
        const axiosConfig={
            ...
            headers:{
                ...
                "Content-Type":"application/json", 
                Origin:
                    "chrome-extension://knnliglhgkmlblppdejchidfihjnockl",// Header origin acak 
                "User-Agent":getRandomUserAgent()
            }
        };
        ...
        // Jika server membatasi permintaan. 
    } catch(error){ 
        if(error.response?.status===429){ 
            await handleTooManyRequests(60)// Retry setelah delay 
            return getUserStats(tokens ,config ,retryCount +1);// Retry dengan menginkremen retryCount
        }

        coloredLog(`Error mendapatkan statistik pengguna:${error.message}`,'ERROR'); 
        if(error.message.includes('WAF') && retryCount < config.waf.maxRetries){ 
            const delay=config.waf.retryIntervalSeconds *1000;

            coloredLog(`WAF terdeteksi ,mencoba lagi setelah${config.waf.retryIntervalSeconds} detik(percobaan${retryCount + 1})`,'WARN');

            await new Promise(resolve=>setTimeout(resolve ,delay));

            return getUserStats(tokens ,config ,retryCount + 1);// Rekursi dengan retryCount bertambah 
        }
        throw error;
    }
}
...
if(!isMainThread){ 
    ...
    async function validateAndSend(){ 
        try { 
            ...
            await sendValidation(tokens ,priceData?.msg_hash ,isValid ,config); 
                ...
                batch.forEach(priceData=>{ 
                    workers.push(new Promise((resolve)=>{ 
                        const worker=new Worker(__filename,{ 
                            workerData:{ priceData, tokens, config }
                        });
        ...
        for (const account of userConfig.accounts) {
            const config = {
                cognito: {
                    ...
                    waf: userConfig.waf // Memastikan konfigurasi WAF ada di sini
                },
            ...
            try {
                ...
                runValidationProcess(tokenManager, config); // Memulai proses validasi
            } catch (error) {
